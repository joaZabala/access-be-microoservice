...
public class AuthService implements IAuthService {
    ...
    public AccessDTO authorizeVisitor(AccessDTO accessDTO, Long guardID) {
        ...
        Optional<AuthEntity> optionalAuth = authRepository.findById(accessDTO.getAuthId());

        AuthEntity authEntity = null;

        Boolean isNotified = false;

        if (accessDTO.getAction() == ActionTypes.ENTRY && optionalAuth.isEmpty()) {
        ...
        if (!optionalAuth.isEmpty()) {
            ...
            if ((authEntity.getVisitorType() == VisitorType.WORKER
                    || authEntity.getVisitorType() == VisitorType.EMPLOYEE)
                    && accessDTO.getAction() == ActionTypes.ENTRY) {
