...
class AuthServiceTest {
    ...
    @Test
    void deleteAllAuthorizationTest() {

        visitorDTO.setVisitorId(1L);
        visitorDTO.setLastName("Pipino");
        visitorDTO.setLastName("Maria");
        visitorDTO.setDocNumber(123456L);
        visitorDTO.setDocumentType(DocumentType.DNI);
        visitorDTO.setActive(true);
        ...
        authEntity.setActive(true);
        authEntity.setPlotId(1L);
        authEntity.setPlotId(10L);


        authEntities.add(authEntity);
        mockAuthDTO.setActive(false);

        when(visitorService.getVisitorByDocNumberAndDocumentType(123456L, DocumentType.DNI)).thenReturn(visitorDTO);
        when(modelMapper.map(any(VisitorDTO.class), eq(VisitorEntity.class))).thenReturn(visitorEntity);
        when(authRepository.findByVisitor(visitorEntity)).thenReturn(authEntities);
        when(authRepository.save(any(AuthEntity.class))).thenReturn(authEntity);
        when(modelMapper.map(any(AuthEntity.class), eq(AuthDTO.class))).thenReturn(mockAuthDTO);

        List<AuthDTO> list =  authService.deleteAllAuthorizationsByDocNumber(123456L, DocumentType.DNI);
    ...
    @Test
    void deleteAllAuthorizationExceptionTest(){
        visitorDTO.setVisitorId(1L);
        visitorDTO.setLastName("Pipino");
        visitorDTO.setLastName("Maria");
        visitorDTO.setDocNumber(123456L);
        ...
        when(visitorService.getVisitorByDocNumberAndDocumentType(123456L , DocumentType.DNI)).thenReturn(null);
        when(modelMapper.map(any(VisitorDTO.class), eq(VisitorEntity.class))).thenReturn(visitorEntity);
        when(authRepository.findByVisitor(visitorEntity)).thenReturn(Collections.emptyList());

        EntityNotFoundException exception = assertThrows(EntityNotFoundException.class, () ->
                authService.deleteAllAuthorizationsByDocNumber(123456L , DocumentType.DNI)
        );
        assertEquals("No se encontraron autorizaciones para el DNI 123456", exception.getMessage());
    }
}
